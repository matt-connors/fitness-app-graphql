...
const main = async () => {
    ...
    // Create a set to track which routineId-exerciseId pairs have been used
    const usedPairs = new Set();

    // Track which predefined pairs will be created
    const predefinedPairs = [
        { routineId: routine[0].id, exerciseId: exercise[0].id }, // Beginner Strength - Bench Press
        { routineId: routine[0].id, exerciseId: exercise[1].id }, // Beginner Strength - Squat
        { routineId: routine[1].id, exerciseId: exercise[2].id }  // Core Mobility - Pull-Up
    ];

    // Add predefined pairs to the used set
    predefinedPairs.forEach(pair => {
        usedPairs.add(`${pair.routineId}-${pair.exerciseId}`);
    });

    // Create an array of routineExercise objects to seed
    const routineExercises = [];

    // Add predefined routine exercises
    ...
    const randomPairsCount = 12;
    let uniquePairsCreated = 0;
    let attempts = 0;
    const maxAttempts = 100; // Prevent infinite loop

    while (uniquePairsCreated < randomPairsCount && attempts < maxAttempts) {
        attempts++;

        const routineId = routine[faker.number.int({ min: 0, max: routine.length - 1 })].id;
        const exerciseId = exercise[faker.number.int({ min: 0, max: exercise.length - 1 })].id;
        const pairKey = `${routineId}-${exerciseId}`;
        ...
        // Add the random routine exercise
        routineExercises.push({
            routineId,
            exerciseId,
    ...
    if (attempts >= maxAttempts) {
        console.log(`Reached max attempts (${maxAttempts}) while trying to create unique pairs`);
    }

    // Seed the routine exercises using the array
    await seed.routineExercise(x => x(routineExercises.length, ({ index }) =>
